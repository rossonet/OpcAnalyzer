...
public final class JIVariant implements Serializable {
	...
	static {
		...
		outTypesMap.put(JIUnsignedByte.class,
				JIUnsignedFactory.getUnsigned(new Short((short) 0), JIFlags.FLAG_REPRESENTATION_UNSIGNED_BYTE));
		outTypesMap.put(JIUnsignedShort.class,
				JIUnsignedFactory.getUnsigned(new Integer(0), JIFlags.FLAG_REPRESENTATION_UNSIGNED_SHORT));
		outTypesMap.put(JIUnsignedInteger.class,
				JIUnsignedFactory.getUnsigned(new Long(0), JIFlags.FLAG_REPRESENTATION_UNSIGNED_INT));
	...
	public static JIVariant OUTPARAMforType(final Class c, final boolean isArray) {
		...
		if (!isArray) {
			...
			if (c.equals(IJIDispatch.class)) {
				final IJIComObject[] arry = new IJIComObject[] {
						new JIComObjectImpl(null, new JIInterfacePointer(null, -1, null)) };
				variant = new JIVariant(new JIArray(arry, true), true);
				variant.setFlag(JIFlags.FLAG_REPRESENTATION_IDISPATCH_NULL_FOR_OUT
						| JIFlags.FLAG_REPRESENTATION_SET_JIINTERFACEPTR_NULL_FOR_VARIANT);
			} else if (c.equals(IJIComObject.class)) {
				final IJIComObject[] arry = new IJIComObject[] {
						new JIComObjectImpl(null, new JIInterfacePointer(null, -1, null)) };
				variant = new JIVariant(new JIArray(arry, true), true);
				variant.setFlag(JIFlags.FLAG_REPRESENTATION_IUNKNOWN_NULL_FOR_OUT
						| JIFlags.FLAG_REPRESENTATION_SET_JIINTERFACEPTR_NULL_FOR_VARIANT);
			} else if (c.equals(JIVariant.class)) {
				return VARIANTARRAY();
	...
	/**
	 * EMPTY <code>VARIANT</code>. This is not Thread Safe , hence a new instance
	...
	/**
	 * <code>VARIANT</code> for <code>([out] IUnknown*)</code>. This is not Thread
	 * Safe , hence a new instance must be taken each time.
	 */
	public static JIVariant OUT_IUNKNOWN() {
		...
		retval.setFlag(JIFlags.FLAG_REPRESENTATION_IUNKNOWN_NULL_FOR_OUT
				| JIFlags.FLAG_REPRESENTATION_SET_JIINTERFACEPTR_NULL_FOR_VARIANT);
		return retval;
	}

	/**
	 * <code>VARIANT</code> for <code>([out] IDispatch*)</code>. This is not Thread
	 * Safe , hence a new instance must be taken each time. <br/>
	 * Note that this must also be used when the interface pointer is a subclass of
	 * <code>IDispatch</code> i.e. supports automation (or is a
	 * <code>dispinterface</code>).
	 */
	public static JIVariant OUT_IDISPATCH() {
		final JIVariant retval = new JIVariant(new JIComObjectImpl(null, new JIInterfacePointer(null, -1, null)), true);
		retval.setFlag(JIFlags.FLAG_REPRESENTATION_IDISPATCH_NULL_FOR_OUT
				| JIFlags.FLAG_REPRESENTATION_SET_JIINTERFACEPTR_NULL_FOR_VARIANT);
		return retval;
	}
	...
	/**
	 * NULL <code>VARIANT</code> . This is not Thread Safe , hence a new instance
	 * must be taken each time.
	 */
	...
	/**
	 * OPTIONAL PARAM. Pass this when a parameter is <code>[optional]</code> for a
	 * COM call. This is not Thread Safe , hence a new instance must be taken each
	 * time.
	 */
	 ...
	 * <code>[in, out] SAFEARRAY(BSTR) *p</code>. The return value can directly be
	...
	/**
	 * Helper method for creating an array of <code>VARIANT</code>s , IDL signature
	 * <code>[in, out] SAFEARRAY(VARIANT) *p</code> OR
	 * <code>[in,out] VARIANT *pArray</code>. The return value can directly be used
	 * in an <code>IJIDispatch</code> call.
	 * 
	 * @return
	 */
	...
	// /**
	// * Setting up a <code>VARIANT</code> with an object. Used via serializing the
	// <code>VARIANT</code>.
	// *
	// * @param obj
	// */
	...
	// /** For internal use only !. Please do not call this directly from outside.
	...
	private void init(final Object obj, final boolean isByRef) {
		if (obj != null && obj.getClass().isArray()) {
			throw new IllegalArgumentException(
					JISystem.getLocalizedMessage(JIErrorCodes.JI_VARIANT_ONLY_JIARRAY_EXCEPTED));
		}
		...
		// this case comes only for SCODE and EMPTY, and in these cases the isByRef flag
		// will be set in the
	 ...
	 * serializing the <code>VARIANT</code>. Used when the variant type is VT_I4.
	 * 
	 * @param value
	 ...
	 * serializing the <code>VARIANT</code>. Used when the variant type is VT_I8.
	 * 
	 * @param value
	 * @param isByRef <code>true</code> if the value is to be represented as a
	 *                pointer.
	 ...
	 * serializing the <code>VARIANT</code>. Internally a <code>JIString</code> is
	 * formed with it's default type <code>BSTR</code>.
	 * 
	 * @param value
	 * @param isByRef <code>true</code> if the value is to be represented as a
	 *                pointer. BSTR*
	...
	// /**Setting up a <code>VARIANT</code> with a IJIDispatch. Used via serializing
	...
	/**
	 * Setting up a <code>VARIANT</code> with an <code>IJIComObject</code>. Used via
	 * serializing the <code>VARIANT</code>.
	 * 
	 * @param value
	 * @param isByRef <code>true</code> if the value is to be represented as a
	...
	/**
	 * Setting up a <code>VARIANT</code> with a <code>SCODE</code> value and it's
	 * <code>errorCode</code>. Used via serializing the <code>VARIANT</code>.
	 * 
	 * @param value
	 * @param errorCode
	 * @param isByRef   <code>true</code> if the value is to be represented as a
	...
	/**
	 * Setting up a <code>VARIANT</code> with an <code>java.util.Date</code>. Used
	...
	/**
	 * Setting up a <code>VARIANT</code> with a <code>JICurrency</code>. Used via
	 * serializing the <code>VARIANT</code>.
	 * 
	 * @param value
	 */
	...
	/**
	 * Setting up a <code>VARIANT</code> with an <code>EMPTY</code> value. Used via
	 * serializing the <code>VARIANT</code>.
	 * 
	 * @param value
	 */
	...
	/**
	 * Setting up a <code>VARIANT</code> with a <code>NULL</code> value. Used via
	 * serializing the <code>VARIANT</code>.
	 * 
	 * @param value
	 */
	...
	private void initArrays(final JIArray array, final boolean isByRef, int FLAG) {
		...
		case 2:
			/*
			 * The 2 dimensional array is serialized like this first the index [0,0] and
			 * then [1,0] then [0,1] then [1,1], then [0,2] then [1,2] and so on . so what i
			 * will do here is that create a single dimension flat array of the members in
			 * the order specified above, after examining this Object[][] and let the 1
			 * dimension serializing logic take over.
			 */
			...
			int k = 0;
			newArrayObj = (Object[]) Array.newInstance(subArray.getClass().getComponentType(),
					array.getNumElementsInAllDimensions());
		...
		try {
			...
			if (c.equals(JIVariant.class)) {
				...
				// JStruct and JIUnions are expected to be encapsulated within pointers...they
				// usually are :)
				elementSize = JIMarshalUnMarshalHelper.getLengthInBytes(c, null,
						c == Boolean.class ? JIFlags.FLAG_REPRESENTATION_VARIANT_BOOL : JIFlags.FLAG_NULL); // All other
																											// types,
																											// basic
																											// types
			}
			...
			safeArray.addMember(new Short(JIVariant.getSupportedType(c, FLAG).shortValue()));// variant array,
																								// safearrayunion
	...
	/**
	 * Setting up a <code>VARIANT</code> with a <code>unsigned</code> value. Used
	 * via serializing the <code>VARIANT</code>.
	 * 
	 * @param number
	 */
	...
	// /**Retrieves the contained object as IJIComObject. Return value can be safely
	// typecasted to the expected type. for e.g. :- If expected type is an
	// IJIDispatch ,
	// * then the return value can be safely type casted to it.
	// *
	// * @param template <code>IJIComObject</code> whose basic parameters such as
	// <code>JIComServer</code> will be used while creating the new Instance.
	// * @return
	// * @throws JIException
	// * @deprecated
	// */
	// public IJIComObject getObjectAsComObject(IJIComObject template) throws
	// JIException
	// {
	// checkValidity();
	// return
	// JIObjectFactory.createCOMInstance(template,((VariantBody)member.getReferent()).getObjectAsInterfacePointer());
	// }
	 ...
	 * <code>JIObjectFactory.narrowInstance(IJIComObject)</code> to get to the right
	 * type.
	 * 
	 * @return
	 * @throws JIException
	...
	/**
	 * Retrieves the contained object as <code>long</code>, used when the expected
	 * type is VT_I8.
	 * 
	 * @return
	 * @throws JIException
	...
	void encode(final NetworkDataRepresentation ndr, final List defferedPointers, final int FLAG) {
		...
		// as soon as this call finishes a call will be given from JICallobject for it's
		// variantbody.
		JIMarshalUnMarshalHelper.serialize(ndr, this.member.getClass(), this.member, defferedPointers, FLAG);
	}

	static JIVariant decode(final NetworkDataRepresentation ndr, final List defferedPointers, final int FLAG,
		...
		// as soon as this call finishes a call will be given from JICallobject for it's
		// variantbody.
		variant.member = (JIPointer) JIMarshalUnMarshalHelper.deSerialize(ndr, ref, defferedPointers, FLAG,
				additionalData);
		return variant;
	}
...
class VariantBody implements Serializable {
	...
	private VariantBody(final Object referent, final boolean isByRef, final int dataType) {
		...
		if (this.obj instanceof JIString
				&& ((JIString) this.obj).getType() != JIFlags.FLAG_REPRESENTATION_STRING_BSTR) {
		...
		// {
		// JISystem.getLogger().info("In VariantBody(Object,boolean,int) : dataType is "
		// + dataType + " , referent class is " + this.obj.getClass() + " , byRef is " +
		// isByRef);
		// }
	...
	/**
	 * Setting up a <code>VARIANT</code> with a NULL value. Used via serializing the
	 * <code>VARIANT</code>.
	 * 
	 * @param value
	 */
	...
	/**
	 * Setting up a <code>VARIANT</code> with a SCODE value and it's errorCode. Used
	 * via serializing the <code>VARIANT</code>.
	 * 
	 * @param value
	 * @param errorCode
	...
	VariantBody(final JIStruct safeArray, final Class nestedClass, final boolean is2Dimensional, final boolean isByRef,
			final int FLAG) {
		this.FLAG = FLAG;
		// can't convert the array here , since this will have deffered pointers which
		// may not be complete.
		...
		// please remember JIVariant is a pointer and VariantBody is just the referent
		// part of that.
	...
	// VariantBody(JIArray obj, Class nestedClass, boolean is2Dimensional,boolean
	// isByRef)
	// {
	//
	// this.objArray = obj;
	...
	//
	// //for an unsupported type this could be null
	// //but then this is my bug, any thread entering this ctor , will support a
	// type.
	// this.isByRef = isByRef;
	...
	JIArray getArray() throws JIException {
		JIArray retVal = null;
		// TODO convert it to the right type based on the variantType before returning
		// it.
		// everything is sent encapsulated in a variant(in safearray) , so an Integer[]
		// will
		// go as a variant array for each integer, only the variantType = arry of ints.
		// so convert the
		// array in the right format before returning it to the user. That is he must
		// get Int[] within a JIArray
		// back.
		if (this.safeArrayStruct != null) {
			...
			if (this.is2Dimensional) {
				...
				// correct the array here , i.e reform the 2 dimensional array before returning
				// back.
				...
				if (this.nestedArraysRealClass != null) {
					...
					for (int i = 0; i < obj.length; i++) {
						...
						// else
						// {
						// Array.set(obj2,i,((JIVariant[])obj)[i].getObject());//should be the native
						// type
						// }
	...
	void encode(final NetworkDataRepresentation ndr, final List defferedPointers, int FLAG) {
		...
		// try
		{
			...
			if ((FLAG
					& JIFlags.FLAG_REPRESENTATION_IDISPATCH_NULL_FOR_OUT) == JIFlags.FLAG_REPRESENTATION_IDISPATCH_NULL_FOR_OUT) {
			...
			if (this.isByRef) {
				...
				if (this.isArray) // object arrays will come here....
				{
					...
					// no idea what these flags are but 0x10 is for variant, 0x8 for date, and 0x4
					// is for others
					switch (this.type) {
			...
			// we should not use the deffered pointers here, but pass our own one, so that
			...
			// making changes to write the deffered pointers here itself , since we need to
			// put the entire Variant completed to the length
			// as in varType.
			int x = 0;
			...
			// {
			// //SafeArray have the alignment rule , that all Size <=4 are aligned by 4 and
			// size 8 is aligned by 8.
			...
			// else
			// {
			// ndr.writeOctetArray(new byte[(length - (int)index)],0,(int)(length -
			// (int)index));
			// }
			// }
			// }
	...
	// }
	// else
	// if(c.equals(Long.class) || c.equals(Double.class) || c.equals(Date.class) ||
	// c.equals(JICurrency.class))
	// {
	// length = 4;
	...
	// }
	//
	// //20 is of variant, 4+4+4+4 of bstr(user,maxlen,actlen,offset) , (strlen*2)
	// of the actual array
	...
	// {
	// double value =
	// ((IJIComObject)obj).internal_getInterfacePointer().getLength();
	// if (isByRef)
	// {
	// value = value + 4;
	// }
	//
	// value = value + 20 + 4 + 4 + 4; //20 of variant , 4 of the ptr, 4 of max
	// count, 4 of actual count
	//
	// double d = value%8.0;
	// length = (int)value/8;
	// if (d != 0.0)
	...
	private int getMaxLength2(final Class c, final Object obj) {
		...
		// proper type 3 elements and not EMPTY,NULL,SCODE since these are parts of
		// Variant.
		...
		} else if (c.equals(Long.class) || c.equals(Double.class) || c.equals(Date.class)
				|| c.equals(JICurrency.class)) {
			length = 8;
		} else if (c.equals(JIString.class)) {
			length = JIMarshalUnMarshalHelper.getLengthInBytes(c, obj, this.FLAG);
	...
	private int getArrayLengthForVarType() throws JIException {
		// now the array will be of variants, nestedArraysRealClass identifies the class
		// itself
		...
		final boolean isVariantArray = (((Short) this.safeArrayStruct.getMember(1)).shortValue()
				& JIVariant.FADF_VARIANT) == JIVariant.FADF_VARIANT ? true : false;
		...
		if (array != null) {
			...
			if (isVariantArray) {
				...
				for (final Object element : array) {
					final JIVariant variant = (JIVariant) element;
					length += variant.getLengthInBytes(this.FLAG);// *
																	// 8;//((VariantBody)(variant.member.getReferent())).variantType
																	// * 8;
				}
				...
				for (final Object element : array) {
					length += getMaxLength2(element.getClass(), element);
				}
			}
		} else {
	...
	static VariantBody decode(final NetworkDataRepresentation ndr, final List defferedPointers, int FLAG,
			final Map additionalData) {
		...
		if ((variantType & JIVariant.VT_ARRAY) == 0x2000) {
			...
			final JIStruct safeArray = getDecodedValueAsArray(ndr, varDefferedPointers,
					variantType & ~JIVariant.VT_ARRAY, isByRef, additionalData, FLAG);
			...
			if (safeArray != null) {
				variant = new VariantBody(safeArray,
						JIVariant.getSupportedClass(new Integer(type2 & ~JIVariant.VT_ARRAY)),
						((Object[]) ((JIArray) safeArray.getMember(8)).getArrayInstance()).length > 1 ? true : false,
						isByRef, flagofFlags);
			} else {
				variant = new VariantBody(null, JIVariant.getSupportedClass(new Integer(type2 & ~JIVariant.VT_ARRAY)),
						false, isByRef, flagofFlags);
			}
			...
			final boolean isByRef = (variantType & JIVariant.VT_BYREF) == 0 ? false : true;
			variant = new VariantBody(
					getDecodedValue(ndr, varDefferedPointers, variantType, isByRef, additionalData, FLAG), isByRef,
					variantType);
		...
		while (x < varDefferedPointers.size()) {
			...
			final ArrayList newList = new ArrayList();
			final JIPointer replacement = (JIPointer) JIMarshalUnMarshalHelper.deSerialize(ndr,
					varDefferedPointers.get(x), newList, FLAG, additionalData);
			((JIPointer) varDefferedPointers.get(x)).replaceSelfWithNewPointer(replacement); // this should replace the
	...
	// Variants need specialised handling and the standard serializers may or maynot
	// be used.
	...
	private int getVarType(final Class c, final Object obj) {
		...
		if (c != null) {
			...
			if (type2 != null) {
				...
				logger.warn("In getVarType: Unsupported Type found ! {} , please add this to the supportedType map ! ",
						c);
	...
	private static Object getDecodedValue(final NetworkDataRepresentation ndr, final List defferedPointers, int type,
			final boolean isByRef, final Map additionalData, final int FLAG) {
		...
		if (c != null) {
			...
			if (c.equals(VariantBody.SCODE.class)) {
				...
				// have read 20 bytes
				// JIMarshalUnMarshalHelper.deSerialize(ndr,Integer.class,null,JIFlags.FLAG_NULL);//read
				// the last 4 bytes, since there could be parameters before this.
				obj = NULL;
			...
			} else if (c.equals(VariantBody.EMPTY.class)) // empty is 20 bytes
			{
				...
				obj = JIMarshalUnMarshalHelper.deSerialize(ndr, c, defferedPointers,
						FLAG | JIFlags.FLAG_REPRESENTATION_VARIANT_BOOL, additionalData);
			} else {
	...
	private static JIStruct getDecodedValueAsArray(final NetworkDataRepresentation ndr, final List defferedPointers,
			int type, final boolean isByRef, final Map additionalData, int FLAG) {
		// int newFLAG = FLAG;
		if (isByRef) {
			ndr.readUnsignedLong();// read the pointer
		...
		try {
			...
			if (c == null) {
				logger.warn(
						"From JIVariant: while decoding an Array, type {} , was not found in supportedTypes_classes map , hence using JIVariant instead...",
						type);
			...
			// then why did I typify this thing to it's class (like JIString), it produces
			// an
			...
			safeArray = (JIStruct) JIMarshalUnMarshalHelper.deSerialize(ndr, safeArray, defferedPointers, FLAG,
					additionalData);

			// now set the right class after examining the flags , only set for
			// JIVariant.class now., the BSTR would already be set previously.
			final Short features = (Short) safeArray.getMember(1);
			// this condition is being kept in the front since the feature flags can be a
			// combination of FADF_VARIANT and the
			// other flags , in which case the Variant takes priority (since they will all
			// be wrapped as variants).
	...
	private void setValue(final NetworkDataRepresentation ndr, final Object obj, final List defferedPointers,
			final int FLAG) {
		if (this.isNull) {
		...
		if (obj != null) {
			...
			// if (c.equals(Boolean.class))
			// {
			// ndr.writeUnsignedShort(((Boolean)obj).booleanValue() == true ? 0xFFFF:
			// 0x0000);
	...
	int getLengthInBytes() {
		...
		if (this.isArray) {
			int length = 0;
			// JIArray objArray =
			// (JIArray)((JIPointer)safeArrayStruct.getMember(7)).getReferent();
