...
final class JIMarshalUnMarshalHelper {
	...
	// TODO This is very important , please note that arrays in C++ have a fixed
	// size and unlike Java have to be
	// declared with there Max index right in the beginning. therefore all arrays
	// (of any type) , will
	// already come padded here to there Max size., this has to be ensured by the
	// caller
	// Basically the index on COMs side should match with the array length
	// here...otherwise exception
	...
	static {
		...
		mapOfSerializers.put(Long.class, new JIMarshalUnMarshalHelper.LongImpl());// LONG , 8 bytes, written as 4+4 in
																					// LE.
		...
		// mapOfSerializers.put(IJIUnsigned.class,new
		// JIMarshalUnMarshalHelper.JIUnsignedImpl());
	...
	static void serialize(final NetworkDataRepresentation ndr, Class c, final Object value, final List defferedPointers,
			final int FLAG) {
		if (c.equals(JIArray.class)) {
			...
			// JIInterfacePointer interfacePointer = new
			// JIInterfacePointer(IJIDispatch.IID,unknown.getInterfacePointer());
			// interfacePointer.encode(ndr,defferedPointers,FLAG);
			// return ;
			// }
			//
			// if (c.equals(JIComObjectImpl.class) || c.equals(IJIComObject.class) ||
			// c.equals(IJIUnknown.class))
			// {
			// JIInterfacePointer interfacePointer =
			// ((IJIComObject)value).getInterfacePointer();
			// interfacePointer.encode(ndr,defferedPointers,FLAG);
			// return ;
			// }
			...
			if (mapOfSerializers.get(c) == null) {
				throw new IllegalStateException(
						MessageFormat.format(JISystem.getLocalizedMessage(JIErrorCodes.JI_UTIL_SERDESER_NOT_FOUND),
								(Object[]) new String[] { c.toString() }));
			}
	...
	static void alignMemberWhileEncoding(final NetworkDataRepresentation ndr, final Class c, final Object obj) {
		...
		if (c.equals(JIStruct.class)) {
			...
			ndr.writeOctetArray(new byte[(int) i], 0, (int) i);
				...
				|| c.equals(Float.class) || c.equals(JIVariant.class) || c.equals(String.class)
	...
	static void alignMemberWhileDecoding(final NetworkDataRepresentation ndr, final Class c, final Object obj) {
		...
		if (c.equals(JIStruct.class)) {
			...
			long i = (long) ((i = Math.round(index % align)) == 0 ? 0 : align - i);
			ndr.readOctetArray(new byte[(int) i], 0, (int) i);
		} else if (c.equals(Integer.class) || c.equals(Float.class) || c.equals(JIVariant.class)
				|| c.equals(String.class) || c.equals(JIPointer.class)) {
			// align with 4 bytes
			long i = (i = Math.round(index % 4.0)) == 0 ? 0 : 4 - i;
			ndr.readOctetArray(new byte[(int) i], 0, (int) i);
		} else if (c.equals(Double.class)) {
	...
	static Object deSerialize(final NetworkDataRepresentation ndr, final Object obj, final List defferedPointers,
			final int FLAG, final Map additionalData) {
		final Class c = obj instanceof Class ? (Class) obj : obj.getClass();
		if (c.equals(JIArray.class)) {
			return ((JIArray) obj).decode(ndr, ((JIArray) obj).getArrayClass(), ((JIArray) obj).getDimensions(),
			...
			if (obj.equals(JIInterfacePointer.class)) {
				final JIInterfacePointer retVal = JIInterfacePointer.decode(ndr, defferedPointers, FLAG,
						additionalData);
				return retVal;
			}
			...
			if (mapOfSerializers.get(obj) == null) {
				throw new IllegalStateException(
						MessageFormat.format(JISystem.getLocalizedMessage(JIErrorCodes.JI_UTIL_SERDESER_NOT_FOUND),
								(Object[]) new String[] { obj.toString() }));
			}
			return ((SerializerDeserializer) mapOfSerializers.get(obj)).deserializeData(ndr, defferedPointers,
					additionalData, FLAG);
		}
	...
	private static class PointerImpl implements SerializerDeserializer {
		...
		@Override
		public void serializeData(final NetworkDataRepresentation ndr, final Object value, final List defferedPointers,
				final int FLAG) {
			throw new IllegalStateException(JISystem.getLocalizedMessage(JIErrorCodes.JI_UTIL_INCORRECT_CALL));
		}

		@Override
		public Object deserializeData(final NetworkDataRepresentation ndr, final List defferedPointers,
				final Map additionalData, final int FLAG) {
			throw new IllegalStateException(JISystem.getLocalizedMessage(JIErrorCodes.JI_UTIL_INCORRECT_CALL));
		}
	...
	private static class JIUnsignedIntImpl implements SerializerDeserializer {
		...
		public void serializeData(final NetworkDataRepresentation ndr, final Object value, final List defferedPointers,
				final int FLAG) {
			JIMarshalUnMarshalHelper.serialize(ndr, Integer.class,
					new Integer(((JIUnsignedInteger) value).getValue().intValue()), null, FLAG);
		}

		@Override
		public Object deserializeData(final NetworkDataRepresentation ndr, final List defferedPointers,
				final Map additionalData, final int FLAG) {
			final Integer b = (Integer) JIMarshalUnMarshalHelper.deSerialize(ndr, Integer.class, null, FLAG,
					additionalData);
			return JIUnsignedFactory.getUnsigned(new Long(b.intValue() & 0xFFFFFFFFL),
	...
	private static class JIUnsignedByteImpl implements SerializerDeserializer {
		...
		public void serializeData(final NetworkDataRepresentation ndr, final Object value, final List defferedPointers,
				final int FLAG) {
			JIMarshalUnMarshalHelper.serialize(ndr, Byte.class,
					new Byte(((JIUnsignedByte) value).getValue().byteValue()), null, FLAG);
		}
		...
		public Object deserializeData(final NetworkDataRepresentation ndr, final List defferedPointers,
			...
			return JIUnsignedFactory.getUnsigned(new Short((short) (b.byteValue() & 0xFF)),
	...
	private static class JIUnsignedShortImpl implements SerializerDeserializer {
		...
		public void serializeData(final NetworkDataRepresentation ndr, final Object value, final List defferedPointers,
				final int FLAG) {
			JIMarshalUnMarshalHelper.serialize(ndr, Short.class,
					new Short(((JIUnsignedShort) value).getValue().shortValue()), null, FLAG);
		}
		...
		public Object deserializeData(final NetworkDataRepresentation ndr, final List defferedPointers,
			...
			return JIUnsignedFactory.getUnsigned(new Integer(b.shortValue() & 0xFFFF),
	...
	//
	// public void serializeData(NetworkDataRepresentation ndr,Object value,List
	// defferedPointers,int FLAG)
	// {
	...
	// default:
	// throw new
	// IllegalStateException(MessageFormat.format(JISystem.getLocalizedMessage(JIErrorCodes.JI_UTIL_SERDESER_NOT_FOUND),new
	// String[]{"IJIUnsigned#" + unsigned.getType()}));
	// }
	//
	// }
	//
	...
	// if ((FLAG & JIFlags.FLAG_REPRESENTATION_UNSIGNED_BYTE) ==
	// JIFlags.FLAG_REPRESENTATION_UNSIGNED_BYTE)
	// {
	// type = JIFlags.FLAG_REPRESENTATION_UNSIGNED_BYTE;
	// }
	// else
	// if ((FLAG & JIFlags.FLAG_REPRESENTATION_UNSIGNED_SHORT) ==
	// JIFlags.FLAG_REPRESENTATION_UNSIGNED_SHORT)
	// {
	// type = JIFlags.FLAG_REPRESENTATION_UNSIGNED_SHORT;
	// }
	// else
	// if ((FLAG & JIFlags.FLAG_REPRESENTATION_UNSIGNED_INT) ==
	// JIFlags.FLAG_REPRESENTATION_UNSIGNED_INT)
	// {
	...
	// {
	// case JIFlags.FLAG_REPRESENTATION_UNSIGNED_BYTE:
	// unsigned = (IJIUnsigned)JIMarshalUnMarshalHelper.deSerialize(ndr,
	// JIUnsignedByte.class, defferedPointers, FLAG, additionalData);
	// break;
	//
	// case JIFlags.FLAG_REPRESENTATION_UNSIGNED_SHORT:
	// unsigned = (IJIUnsigned)JIMarshalUnMarshalHelper.deSerialize(ndr,
	// JIUnsignedShort.class, defferedPointers, FLAG, additionalData);
	// break;
	//
	// case JIFlags.FLAG_REPRESENTATION_UNSIGNED_INT:
	// unsigned = (IJIUnsigned)JIMarshalUnMarshalHelper.deSerialize(ndr,
	// JIUnsignedInteger.class, defferedPointers, FLAG, additionalData);
	// break;
	//
	// default:
	// throw new
	...
	// case JIFlags.FLAG_REPRESENTATION_UNSIGNED_BYTE:
	// length =
	// JIMarshalUnMarshalHelper.getLengthInBytes(JIUnsignedByte.class,value,FLAG);
	// break;
	//
	// case JIFlags.FLAG_REPRESENTATION_UNSIGNED_SHORT:
	// length =
	// JIMarshalUnMarshalHelper.getLengthInBytes(JIUnsignedShort.class,value,FLAG);
	// break;
	//
	// case JIFlags.FLAG_REPRESENTATION_UNSIGNED_INT:
	// length =
	// JIMarshalUnMarshalHelper.getLengthInBytes(JIUnsignedInteger.class,value,FLAG);
	// break;
	//
	// default:
	// throw new
	...
	private static class IJIComObjectSerDer implements SerializerDeserializer {
		...
		public void serializeData(final NetworkDataRepresentation ndr, final Object value, final List defferedPointers,
				final int FLAG) {
			serialize(ndr, JIInterfacePointer.class, ((IJIComObject) value).internal_getInterfacePointer(),
					defferedPointers, FLAG);
		}
		...
		public Object deserializeData(final NetworkDataRepresentation ndr, final List defferedPointers,
			...
			final IJIComObject comObject = new JIComObjectImpl(session, (JIInterfacePointer) deSerialize(ndr,
					JIInterfacePointer.class, defferedPointers, FLAG, additionalData));
	...
	// {
	// throw new
	// IllegalStateException(JISystem.getLocalizedMessage(JIErrorCodes.JI_UTIL_INCORRECT_CALL));
	// }
	//
	// public Object deserializeData(NetworkDataRepresentation ndr,List
	// defferedPointers, Map additionalData, int FLAG)
	...
	private static class ShortImpl implements SerializerDeserializer {
		@Override
		public void serializeData(final NetworkDataRepresentation ndr, Object value, final List defferedPointers,
				final int FLAG) {
			if (value == null) {
	...
	private static class DoubleImpl implements SerializerDeserializer {
		...
		public void serializeData(final NetworkDataRepresentation ndr, Object value, final List defferedPointers,
			...
			ndr.getBuffer().align(8);
			Encdec.enc_doublele(((Double) value).doubleValue(), ndr.getBuffer().getBuffer(),
					ndr.getBuffer().getIndex());
			ndr.getBuffer().advance(8);
	...
	private static class JICurrencyImpl implements SerializerDeserializer {
		...
		public void serializeData(final NetworkDataRepresentation ndr, final Object value, final List defferedPointers,
			...
			// now align by 8 bytes, since this is struct has a hyper, which I don't support
			// yet
			final double index = new Integer(ndr.getBuffer().getIndex()).doubleValue();
			long i = (i = Math.round(index % 8.0)) == 0 ? 0 : 8 - i;
			ndr.writeOctetArray(new byte[(int) i], 0, (int) i);
	...
	private static class DateImpl implements SerializerDeserializer {
		...
		public void serializeData(final NetworkDataRepresentation ndr, final Object value, final List defferedPointers,
			...
			ndr.getBuffer().align(8);
			Encdec.enc_doublele(convertMillisecondsToWindowsTime(((Date) value).getTime()), ndr.getBuffer().getBuffer(),
					ndr.getBuffer().getIndex());
			ndr.getBuffer().advance(8);
		...
		public Object deserializeData(final NetworkDataRepresentation ndr, final List defferedPointers,
				final Map additionalData, final int FLAG) {
			ndr.getBuffer().align(8);
			final Date b = new Date(convertWindowsTimeToMilliseconds(
					Encdec.dec_doublele(ndr.getBuffer().getBuffer(), ndr.getBuffer().getIndex())));
			ndr.getBuffer().advance(8);
			return b;
		}
		...
		/**
		 * FROM JACAOB 1.10. www.danadler.com. Convert a COM time from functions Date(),
		 * Time(), Now() to a Java time (milliseconds). Visual Basic time values are
		 * based to 30.12.1899, Java time values are based to 1.1.1970 (= 0
		 * milliseconds). The difference is added to the Visual Basic value to get the
		 * corresponding Java value. The Visual Basic double value reads: <day count
		 * delta since 30.12.1899>.<1 day percentage fraction>, e.g. "38100.6453" means:
		 * 38100 days since 30.12.1899 plus (24 hours * 0.6453). Example usage:
	...
	private static class StringImpl implements SerializerDeserializer {
		...
		public void serializeData(final NetworkDataRepresentation ndr, final Object value, final List defferedPointers,
			...
			if ((FLAG & JIFlags.FLAG_REPRESENTATION_STRING_LPCTSTR) == JIFlags.FLAG_REPRESENTATION_STRING_LPCTSTR) {
				...
				ndr.writeUnsignedShort(0);
					...
					& JIFlags.FLAG_REPRESENTATION_STRING_LPWSTR) == JIFlags.FLAG_REPRESENTATION_STRING_LPWSTR) {
		...
		public int getLengthInBytes(final Object value, final int FLAG) {
			...
			if ((FLAG & JIFlags.FLAG_REPRESENTATION_STRING_LPCTSTR) == JIFlags.FLAG_REPRESENTATION_STRING_LPCTSTR) {
				length = length + ((String) value).length(); // this is only a character array, no unicode, each char is
																// writen in 1 byte "abcd" --> ab, cd ,00 ; "abcde" -->
																// ab,cd,e0, 00
	...
	private static class JIStringImpl implements SerializerDeserializer {
		...
		public int getLengthInBytes(final Object value, final int FLAG) {
			...
			return length + JIMarshalUnMarshalHelper.getLengthInBytes(String.class, ((JIString) value).getString(),
					((JIString) value).getType() | FLAG);
		}
