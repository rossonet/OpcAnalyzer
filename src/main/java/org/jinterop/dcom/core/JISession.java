...
public final class JISession
{
        ...
        public void run ()
        {
            ...
            try
            {
                ...
                while ( i < listOfSessionsClone.size () )
                {
                    ...
                    synchronized ( mutex )
                    {
                        logger.debug ( "Release_References_TimerTask:[RUN] Session:  {} , listOfDeferencedIpids.size(): {}", session.getSessionIdentifier (), session.listOfDeferencedIpids.size () );
    ...
    public static JISession createSession ( final IJIAuthInfo authInfo )
    {
        ...
        logger.debug ( "Created Session: {}", session.sessionIdentifier );
        return session;
    }
    ...
    public static JISession createSession ()
    {
        ...
        logger.debug ( "Created Session for SSO: {}", session.sessionIdentifier );
    ...
    public static void destroySession ( final JISession session ) throws JIException
    {
        ...
        try
        {
            ...
            logger.debug ( "Destroyed Session: {}", session.sessionIdentifier );
        }
        finally
        {
            synchronized ( mutex )
    ...
    private static void postDestroy ( final JISession session ) throws JIException
    {
        ...
        logger.debug ( "About to destroy {} sessesion which are linked to this session: {}", session.links.size (), session.getSessionIdentifier () );
    ...
    void addToSession ( final IJIComObject comObject, final byte[] oid )
    {
        ...
        logger.debug ( "for IID: {}", comObject.getInterfaceIdentifier () );
    ...
    private void addToSession ( final String IPID, final byte[] oid, final boolean dontping )
    {
        ...
        logger.debug ( "[addToSession] Adding IPID: {} to session: {}", IPID, getSessionIdentifier () );
    }
    ...
    {
        logger.debug ( "releaseRef:Reclaiming from Session: {} , the IPID: {}, numinstances is {}", numinstances, new Object[] { getSessionIdentifier (), IPID, numinstances } );
        ...
        if ( logger.isInfoEnabled () )
        {
            ...
            logger.debug ( "releaseRef: Releasing numinstances {} references of IPID: {} session: {}", new Object[] { numinstances, IPID, getSessionIdentifier () } );
        }
    ...
    {
        logger.debug ( "addDereferencedIpids for session : {} , IPID is: {}", getSessionIdentifier (), IPID );
    ...
    {
        logger.debug ( "In releaseRefs for session : {} , array length is: {}", getSessionIdentifier (), (short) ( (Object[])arrayOfStructs.getArrayInstance () ).length );
