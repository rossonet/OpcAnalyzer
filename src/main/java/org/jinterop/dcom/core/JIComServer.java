...
public final class JIComServer extends Stub {
	...
	/**
	 * <p>
	 * <code>{@link JIClsid}</code> based constructor with the host machine for COM
	 ...
	 *                                  <code>session</code> is associated with
	 *                                  another server already.
	 * @throws IllegalArgumentException raised when either <code>clsid</code> or
	 *                                  <code>session</code> is <code>null</code>.
	 * @throws UnknownHostException
	 */
	public JIComServer(final JIClsid clsid, final JISession session)
			throws IllegalArgumentException, JIException, UnknownHostException {
	 ...
	 * @throws UnknownHostException
	 */
	public JIComServer(final JIClsid clsid, String address, final JISession session)
	...
	/**
	 * <p>
	 * <code>JIProgId</code> based constructor with the host machine for COM server
	 * being <i>LOCALHOST</i>.
	 *
	 ...
	 *                                  <code>session</code> is associated with
	 *                                  another server already.
	 * @throws IllegalArgumentException raised when either <code>progId</code> or
	 *                                  <code>session</code> is <code>null</code>.
	 * @throws UnknownHostException
	 */
	 ...
	 * @throws UnknownHostException
	 */
	public JIComServer(final JIProgId progId, String address, final JISession session)
			throws JIException, UnknownHostException {
		super();
		...
		if (session.isSSOEnabled()) {
			throw new IllegalArgumentException(
					JISystem.getLocalizedMessage(JIErrorCodes.JI_COMSTUB_ILLEGAL_ARGUMENTS2));
		}
	...
	/**
	 * <p>
	 * Instantiates a JIComServer represented by the interfacePointer param. There
	 * are cases where a COM server may hand down a reference to a different COM
	 * server(which may or may not be on the same machine) and we would like to hook
	 * in between. The <code>IJIComObject</code> interface is usable only in the
	 * context of the current JIComServer, but when the interfacePointer is of a
	 * completely different COM server, the JIObjectFactory APIs will not work. The
	 * reason is the interface pointer passed to those APIs expects to belong only
	 * to a single and same COM server (say 'A'). If by any chance, that COM server
	 * passes a reference to you of another COM server (say 'B') on a different
	 * machine, the <code>IJIComObject</code> so returned from
	 * <code>JIObjectFactory</code> APIs will result in "Method not found"
	 * Exceptions (or others) since the pointer returned via that will always place
	 * calls to 'A' instead of 'B'. Under such scenarios you must use this API. This
	 * is not a usual case and for reasons related to nature of DCOM, will be very
	 * well documented in the Developers guide of your COM server.
	 * <p>
	 * The DCOM specs refer to this as the "middleman" case. (Section 3.3.1)
	 * </p>
	 * </p>
	 *
	 * @param session          Please use a new session and not an already bounded
	 *                         one. The
	 *                         <code>JISession.createSession(JISession)</code> can
	 ...
	 *                         adapters (virtual as well) on the Target machine to
	 *                         which this interface pointer belongs, which may get
	 *                         sent as part of the interface pointer and
	 *                         consequently this call will fail since it is a
	 *                         possibility that IP is not reachable via this
	 *                         machine. The developer can send in the valid IP and
	 *                         if found in the interface pointer list will be used
	 *                         to talk to the target machine, overriding the other
	 *                         IP addresses present in the interface pointer. If
	 *                         this IP is not found then the "machine name" binding
	 *                         will be used. If this param is <code>null</code> then
	 *                         the first binding obtained from the interface pointer
	 *                         is used.
	 */
	JIComServer(final JISession session, final JIInterfacePointer interfacePointer, final String ipAddress)
			throws JIException {
		super();
		...
		super.getProperties().setProperty("rpc.socketTimeout",
				new Integer(session.getGlobalSocketTimeout()).toString());
		...
		// if (!targetAddress.equals(""))
		{
			...
			while (i < addressBindings.length) {
				...
				if (binding.getTowerId() != 0x07) // this means, even though I asked for TCPIP something else was
													// supplied, noticed this in win2k.
				{
					i++;
					continue;
				}
				...
				if (index != -1) {
					try {
						...
						if (index != -1
								&& binding.getNetworkAddress().substring(0, index).equalsIgnoreCase(targetAddress)) {
							nameBinding = null;
							break;
						}
					...
					// can only come for the name, saving it incase nothing matches the target
					// address
					nameBinding = binding;
				}
				i++;
		...
		this.oxidResolver = new JIOxidResolver(
				((JIStdObjRef) interfacePointer.getObjectReference(JIInterfacePointer.OBJREF_STANDARD)).getOxid());
		try {
	...
	void addRef_ReleaseRef(final JICallBuilder obj) throws JIException {
		synchronized (this.mutex) {
			...
			// now also set the Object ID for IRemUnknown call this will be the IPID of the
			// returned JIRemActivation or IOxidResolver
	...
	public IJIComObject createInstance() throws JIException {
		if (this.interfacePtrCtor != null) {
			throw new IllegalStateException(
					JISystem.getLocalizedMessage(JIErrorCodes.JI_COMSTUB_WRONGCALLCREATEINSTANCE));
		}
		IJIComObject comObject = null;

		// This method is still essentially serial, since all threads will have to stop
		// at mutex and then
		...
		synchronized (this.mutex) {
			...
			// JIStdObjRef objRef =
			// (JIStdObjRef)(remoteActivation.getMInterfacePointer().getObjectReference(JIInterfacePointer.OBJREF_STANDARD));
			// comObject = getObject(objRef.getIpid(),IJIUnknown.IID);
			comObject = JIFrameworkHelper.instantiateComObject(this.session,
					this.remoteActivation.getMInterfacePointer());
			if (this.remoteActivation.isDual) {
				// IJIComObject comObject2 =
				// getObject(remoteActivation.dispIpid,"00020400-0000-0000-c000-000000000046");
	 ...
	 * <code>JIComServer(JISession,JIInterfacePointer,String)</code> ctor, otherwise
	...
	IJIComObject getInstance() throws JIException {
		...
		synchronized (this.mutex) {
			...
			// JIStdObjRef objRef =
			// (JIStdObjRef)(interfacePtrCtor.getObjectReference(JIInterfacePointer.OBJREF_STANDARD));
	...
	IJIComObject getInterface(final String iid, final String ipidOfTheTargetUnknown) throws JIException {
		...
		// this is still essentially serial, since all threads will have to wait for
		// mutex before
		// entering addToSession.
		synchronized (this.mutex) {
			// now also set the Object ID for IRemUnknown call this will be the IPID of the
			// returned JIRemActivation
			...
			if (!iid.equalsIgnoreCase("00020400-0000-0000-c000-000000000046")) {
				...
				final JIRemUnknown dispatch = new JIRemUnknown(retval.getIpid(),
				...
				if (success) {
					...
					this.session.releaseRef(dispatch.getInterfacePointer().getIPID(),
							((JIStdObjRef) dispatch.getInterfacePointer()
									.getObjectReference(JIInterfacePointer.OBJREF_STANDARD)).getPublicRefs());
				}
			}
		}
	...
	private void init() throws JIException {
		...
		while (i < bindings.length) {
			binding = bindings[i];
			if (binding.getTowerId() != 0x07) // this means, even though I asked for TCPIP something else was supplied,
												// noticed this in win2k.
			{
				i++;
				continue;
			}
			...
			if (index != -1) {
				...
				// can only come for the name, saving it incase nothing matches the target
				// address
				// then we are not sure which is the right IP and which might be virtual, refer
				// to
				// issue faced by Igor.
				nameBinding = binding;
				index = binding.getNetworkAddress().indexOf("[");// this contains the port
	...
	private void initialise(final JIClsid clsid, final String address, final JISession session) throws JIException {
		...
		try {
			...
			if (e.getErrorCode() == 0x80040154) {
				logger.warn(
						"Got the class not registered exception , will attempt setting entries based on status flags...");
				...
				if (JISystem.isAutoRegistrationSet() || clsid.isAutoRegistrationSet()) {
					...
					try {
						...
						if (session.isSSOEnabled()) {
							...
							registry = JIWinRegFactory.getSingleTon()
									.getWinreg(new JIDefaultAuthInfoImpl(session.getDomain(), session.getUserName(),
											session.getPassword()), session.getTargetServer(), true);
						}
						final JIPolicyHandle hkcr = registry.winreg_OpenHKCR();
						JIPolicyHandle key = registry.winreg_CreateKey(hkcr, "CLSID\\{" + this.clsid + "}",
						...
						key = registry.winreg_CreateKey(hkcr, "AppID\\{" + this.clsid + "}",
								IJIWinReg.REG_OPTION_NON_VOLATILE, IJIWinReg.KEY_ALL_ACCESS);
