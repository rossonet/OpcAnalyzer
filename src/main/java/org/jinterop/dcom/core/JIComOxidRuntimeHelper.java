...
final class JIComOxidRuntimeHelper extends Stub
{
    ...
    void startOxid ( final int portNumLocal, final int portNumRemote ) throws IOException
    {
        final Thread oxidResolverThread = new Thread ( new Runnable () {
            ...
            public void run ()
            {
                try
                {
                    logger.debug ( "started startOxid thread: {}", Thread.currentThread ().getName () );
                    attach ();
                ...
                logger.debug ( "terminating startOxid thread: {}", Thread.currentThread ().getName () );
            }
    ...
    Object[] startRemUnknown ( final String baseIID, final String ipidOfRemUnknown, final String ipidOfComponent, final List listOfSupportedInterfaces ) throws IOException
    {
        ...
        final Thread remUnknownThread = new Thread ( remUnknownForThisListener, new Runnable () {
            ...
            public void run ()
            {
                logger.debug ( "started RemUnknown listener thread for : {}", Thread.currentThread ().getName () );
                try
                {
                    ...
                    while ( true )
                    {
                        ...
                        logger.debug ( "RemUnknown listener: Got Connection from {}", socket.getPort () );
                ...
                {
                    logger.debug ( "JIComOxidRuntimeHelper RemUnknownListener" + Thread.currentThread ().getName () + " is purposefully closed by interruption.", e );
                }
                catch ( final IOException e )
                {
                ...
                logger.debug ( "terminating RemUnknownListener thread: {}", Thread.currentThread ().getName () );
            }
...
class OxidResolverImpl extends NdrObject implements IJICOMRuntimeWorker
{
    ...
    {
        logger.debug ( "Oxid Object: SimplePing" );
    ...
    {
        logger.debug ( "Oxid Object: ComplexPing" );
        final byte b[] = JIMarshalUnMarshalHelper.readOctetArrayLE ( ndr, 8 );//setid
    ...
    {
        logger.debug ( "Oxid Object: ServerAlive" );
    ...
    {
        logger.debug ( "Oxid Object: ServerAlive2" );
    ...
    {
        logger.debug ( "Oxid Object: ResolveOxid2" );
