    ...
    public void write ( final NetworkDataRepresentation ndr )
    {
        switch ( this.opnum )
        {
            case 2: //complex ping
                ...
                if ( this.setId == null )
                {
                    logger.debug ( "Complex Ping going for the first time, will get the setId as response of this call" );
                ...
                else
                {
                    ...
                    logger.debug ( "Complex Ping going for setId: {}", byteArrayOutputStream );
                }

                if ( logger.isInfoEnabled () )
                {
                    logger.debug ( "Complex ping going : listOfAdds -> Size : " + this.listOfAdds.size () + " , " + this.listOfAdds );
                    logger.debug ( "listOfDels -> Size : " + this.listOfDels.size () + " , " + this.listOfDels );
                }
            ...
            case 1:// simple ping
                ...
                if ( this.setId != null )
                {
                    ...
                    final ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream ();
                    jcifs.util.Hexdump.hexdump ( new PrintStream ( byteArrayOutputStream ), this.setId, 0, this.setId.length );
                    logger.debug ( "Simple Ping going for setId: {}", byteArrayOutputStream );
                }
                else
                {
                    logger.debug ( "Some error ! Simple ping requested , but has no setID" );
                }
                break;
        ...
        switch ( this.opnum )
        {
            case 2: //complex ping
                ...
                else
                {
                    final ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream ();
                    jcifs.util.Hexdump.hexdump ( new PrintStream ( byteArrayOutputStream ), this.setId, 0, this.setId.length );
                    logger.debug ( "Complex Ping Succeeded,  setId is : {}", byteArrayOutputStream );
                }
            ...
            case 1:// simple ping
                ...
                {
                    logger.debug ( "Simple Ping Succeeded" );
                }
                break;
