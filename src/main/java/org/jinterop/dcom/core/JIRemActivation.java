...
final class JIRemActivation extends NdrObject {
	...
	public void read(final NetworkDataRepresentation ndr) {
		...
		if (skipdual != 0) {
			...
			// now fill the dual string array for oxid bindings, the call to IRemUnknown
			// will be
			// directed to this address and the port in that address.
			this.dualStringArrayForOxid = JIDualStringArray.decode(ndr);
		}
		// get the IPID which will be the "Object" in the call to IRemUknown. This is
		// the IPID of the
		// component which has been specified as the JIClsid. This may differ in
		// multiple invokations of
		// of remote activation as everytime a new object may be created at the server
		// per call. This is all
		...
		if (this.hresult != 0) {
			// System.out.println("EXCEPTION FROM SERVER ! --> " + "0x" +
			// Long.toHexString(hresult).substring(8));
			throw new JIRuntimeException(this.hresult);
		}
		// int numRet = ndr.readUnsignedLong();//Number of interface pointers returned.
		// Currently only 2.

		JIArray array = new JIArray(JIInterfacePointer.class, null, 1, true);
		final ArrayList listOfDefferedPointers = new ArrayList();
		array = (JIArray) JIMarshalUnMarshalHelper.deSerialize(ndr, array, listOfDefferedPointers, JIFlags.FLAG_NULL,
				new HashMap());
		int x = 0;
		...
		while (x < listOfDefferedPointers.size()) {
			...
			final JIPointer replacement = (JIPointer) JIMarshalUnMarshalHelper.deSerialize(ndr,
					listOfDefferedPointers.get(x), newList, JIFlags.FLAG_NULL, null);
			((JIPointer) listOfDefferedPointers.get(x)).replaceSelfWithNewPointer(replacement); // this should replace
																								// the value in the
																								// original place.
			x++;
		...
		if (arrayObjs[1] != null) {
			...
			this.isDual = true;
			// eat this keeping only the IPID for cleanup , let the user perform another
			// queryInterface for this.
	...
	public void write(final NetworkDataRepresentation ndr) {
		...
		ndr.writeUnsignedLong(this.mode); // mode, when object name , interface pointer are not null , this is passed
											// directly to IPersistFile:Load
		...
		// IID of IUnknown , this is hard coded here, standard way of COM is to first
		// get a handle to the IUnknown
