...
public final class JIComRuntimeEndpoint extends ConnectionOrientedEndpoint
{
    ...
    public void processRequests ( final IJICOMRuntimeWorker workerObject, final String baseIID, final List listOfSupportedInterfaces ) throws IOException
    {
        ...
        while ( true )
        {
            ...
            {
                logger.debug ( "processRequests: [JIComRuntimeEndPoint] request : " + Thread.currentThread ().getName () + " , " + request + " workerObject is resolver: " + workerObject.isResolver () );
            }
            ...
            logger.debug ( "processRequests: [JIComRuntimeEndPoint] response : {},{}", Thread.currentThread ().getName (), response );
            //now send the response.
            send ( response );

            if ( workerObject.workerOver () )
            {
                logger.debug ( "processRequests: [JIComRuntimeEndPoint] Worker is over, all IPID references have been released. Thread " + Thread.currentThread ().getName () + " will now exit." );
                break;
            }
        }
