...
public class Group {
	 ...
	 * items and the <em>add</em> them. The spec also says that when a server lets
	 * the item pass validation it must also let them pass the add operation.
	 *
	 ...
	 * @throws JIException        The add operation completely failed. No item was
	 *                            added.
	 * @throws AddFailedException If one or more item could not be added. Item
	 *                            without error where added.
	 */
	public synchronized Map<String, Item> addItems(final String... items) throws JIException, AddFailedException {
		...
		for (final KeyedResult<OPCITEMDEF, OPCITEMRESULT> entry : result) {
			if (entry.getErrorCode() == 0) {
				final Item item = new Item(this, entry.getValue().getServerHandle(), itemDef[i].getClientHandle(),
						entry.getKey().getItemID());
				_log.debug(String.format("create item: " + item));
				addItem(item);
	...
	private synchronized void addItem(final Item item) {
		...
		_log.debug(String.format("Added item: '%s', %d", item.getId(), item.getServerHandle()));
	}
	...
	private synchronized Map<String, Integer> findItems(final String[] items) {
		...
		for (final String item : items) {
			data.put(item, this._itemHandleMap.get(item));
		}
	...
	public synchronized Map<Item, ItemState> read(final boolean device, final Item... items) throws JIException {
		...
		final KeyedResultSet<Integer, OPCITEMSTATE> states = this._syncIO
		...
		for (final KeyedResult<Integer, OPCITEMSTATE> entry : states) {
			...
			final ItemState state = new ItemState(entry.getErrorCode(), entry.getValue().getValue(),
	...
	public String toString() {
		...
		try {
			builder.append("Group [isActive()=").append(isActive()).append(", ");
			if (getName() != null)
