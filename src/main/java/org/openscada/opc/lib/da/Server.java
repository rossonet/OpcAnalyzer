...
public class Server {
	 ...
	 */
	public Group addGroup(final int rate) throws IllegalArgumentException, UnknownHostException, NotConnectedException,
			JIException, DuplicateGroupException {
		return addGroup(null, rate);
	}

	/**
	 * Add a new named group to the server
	 *
	 * @param name The name of the group to use. Must be unique or <code>null</code>
	 *             so that the server creates a unique name.
	 * @return The new group
	 ...
	 */
	public synchronized Group addGroup(final String name, final int rate) throws NotConnectedException,
			IllegalArgumentException, UnknownHostException, JIException, DuplicateGroupException {
		if (!isConnected()) {
			throw new NotConnectedException();
		}

		try {
			final OPCGroupStateMgt groupMgt = this.server.addGroup(name, this.defaultActive, rate, 0,
	...
	protected void cleanup() {
		logger.debug("Destroying DCOM session...");
		...
		final Thread destructor = new Thread(new Runnable() {
			...
			public void run() {
				...
				try {
					...
					logger.debug("Destructed DCOM session");
				} catch (final Throwable e) {
					logger.warn("Failed to destruct DCOM session", e);
				} finally {
					logger.debug(String.format("Session destruction took %s ms", System.currentTimeMillis() - ts));
				}
			}
		...
		destructor.start();
		logger.debug("Destroying DCOM session... forked");
	...
	public synchronized void connect()
		...
		logger.debug(String.format("Socket timeout: %s ", socketTimeout));
		...
		try {
			...
			logger.debug("Unknown host when connecting to server", e);
			cleanup();
			throw e;
		} catch (final JIException e) {
	...
	/**
	 * Gets the scheduler for the server. Note that this scheduler might get blocked
	 * for a short time if the connection breaks. It should not be used for time
	 * critical operations.
	 *
	...
	public OPCSERVERSTATUS getServerState() {
		try {
			...
			logger.debug("Server connection failed", e);
			dispose();
			return null;
		}
	}
